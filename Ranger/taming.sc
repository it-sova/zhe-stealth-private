   ...
   begin
      ...
      begin
         ...
         begin
            ...
            WaitJournalLine(Now, 'You have succesfully|You failed', 30000);
...
begin
   ...
   begin
      ...
      WaitTargetObject(Zver[tmp]);
      UseSkill('Animal Taming');
      WaitJournalLine(Now,'You have succesfully|You fail', 45000);
      UOsay('all release');
      i := i + 1;
      end;
   End;
...
begin
   SetARStatus(true);
   Char[1]:='Tinybit'; {Ник первого игрока }
   Char[2]:='Delphina'; {Ник второго игрока }
   Zver[1]:=$00285C6B; {Айди животных }
   Zver[2]:=$002565F2; {Айди животных }
   Zver[3]:=$00241620; {Айди животных }
   Zver[4]:=$002888BD; {Айди животных }
   Case CharName Of
      Char[1] : First();
      Char[2] : Second();
